...
T_simulazione = 3;
T_simulazione_dd = 1000; % per far vedere il periodo di dd nel plot
passo = 0.7e-6; % sample rate doppio rispetto alla frequenza massima 
passo_dd = 0.01;

global tt tt_dd ww;
tt = (0:passo:T_simulazione);
tt_dd=(0:passo_dd:T_simulazione_dd);
...
for k=[1:1:4]
    ...
    dd_plot = dd_plot + DD*sin(wd*k*tt_dd); 
    nn = nn + NN*sin(wn*k*tt); 
end

% uscite dei rumori
y_d = lsim(SS, dd, tt);
y_d_plot=lsim(SS, dd_plot, tt_dd);
y_n = lsim(-FF, nn, tt);

% plot per d
figure(7);
hold on, grid on, zoom on
plot(tt_dd,dd_plot,'m')
plot(tt_dd,y_d_plot,'b')
legend('dd','y_d')
